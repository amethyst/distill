...
fn calc_import_hash<K>(importer_options: &Box<SerdeObj>, importer_state: &Box<SerdeObj>, source_hash: u64, asset_id: &K) where K: AsRef<[u8]> {
...
) -> Result<Vec<ImportedAsset>, FileError> {
...
impl FileAssetSource {
    pub fn new(tracker: &Arc<FileTracker>, hub: &Arc<AssetHub>, db: &Arc<Environment>) -> Result<FileAssetSource> {
    ...
    fn handle_update(&self) -> Result<()> {
        ...
        let mut processing_results: Vec<(HashedAssetFilePair, Result<()>)> = Vec::new();
        {
                    ...
                    Ok(_) => {
                        let mut skip_ack_dirty = false;
                        {
                            let check_file_state =
                                |s: &Option<&FileState>| -> Result<bool> {
                                    match s {
                                        Some(source) => {
